...
class CustomImage():
    def __init__(self , image = [], loaded = False):
        ...
        if (len(image) != 0 ):
            ...
            self.modified_image_fourier_components_real = np.real(self.__modified_image_fourier_components.T)
            self.modified_image_fourier_components_imag = np.imag(self.__modified_image_fourier_components.T)
    @property
